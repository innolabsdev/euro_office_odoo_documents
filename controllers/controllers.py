...
from odoo.addons.euro_office_odoo.controllers.controllers import EuroOffice_Connector
...
class EuroOfficeDocuments_Connector(http.Controller):
    @http.route("/euro_office/documents/file/create", auth="user", methods=["POST"], type="jsonrpc")
    def post_file_create(self, folder_id, supported_format, title, url=None):
        ...
        try:
            ...
            request.env["euro_office.odoo.documents.access.user"].create(
                {
                    "document_id": document.id,
...
class EuroOfficeDocuments_Inherited_Connector(EuroOffice_Connector):
    @http.route(["/euro_office/documents/share/<access_token>/"], type="http", auth="public")
    def render_shared_document_editor(self, access_token=None):
        try:
            ...
            return request.render(
                "euro_office_odoo.euro_office_editor", self.prepare_share_editor(document, access_token)
            )
    ...
    @http.route("/euro_office/editor/document/<int:document_id>", auth="public", type="http", website=True)
    def render_document_editor(self, document_id, access_token=None):
        return request.render(
            "euro_office_odoo.euro_office_editor", self.prepare_document_editor(document_id, access_token)
        )
    ...
    def prepare_share_editor(self, document, access_token):
        role = None
        access = (
            request.env["euro_office.odoo.documents.access"].sudo().search([("document_id", "=", document.id)], limit=1)
        )
        if access:
        ...
        if role and role != "view":
            ...
            root_config["editorConfig"]["callbackUrl"] = (
                odoo_url + "euro_office/documents/share/callback/" + access_token + "/" + security_token
            )
    ...
    @http.route(
        "/euro_office/documents/share/callback/<access_token>/<oo_security_token>",
        auth="public",
        methods=["POST"],
        type="http",
        csrf=False,
    ...
    def share_callback(self, access_token, oo_security_token):
        ...
        try:
            ...
            access = (
                request.env["euro_office.odoo.documents.access"]
                .sudo()
...
class EuroOfficeShareRoute(ShareRoute):
    @http.route("/documents/<access_token>", type="http", auth="public")
    def documents_home(self, access_token):
        response = super(EuroOfficeShareRoute, self).documents_home(access_token)  # noqa: UP008
        ...
        if document_sudo.type == "binary" and document_sudo.attachment_id:
            ...
            if can_view:
                qcontext["euro_office_supported"] = True
        ...
        if document_sudo.type == "folder":
            ...
            for document in sub_documents_sudo:
                data.append({"document": document, "euro_office_supported": file_utils.can_view(document.name)})
            qcontext["euro_office_supported"] = data
